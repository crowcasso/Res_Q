 ...
 */
//@TeleOp
public class Nasic6_3 extends OpMode {
    ...
    private final long BUCKET_EXTRACT_TIME = 700;
    ...
    private void stateOfTheArm() {
        if( gamepad1.b && !prevG2B ) {
            if(armState == ArmState.RESTING_DOWN) {
                isWristUp = !isWristUp;
                if(isWristUp) wristPosition = STARTING_WRIST_POSITION;
                else wristPosition = 0;
            }
            if(armState == ArmState.RESTING_UP) {
                isWristUp = !isWristUp;
                if(isWristUp) wristPosition = ENDING_WRIST_POSITION;
                else wristPosition = STARTING_WRIST_POSITION;
            }
        }
    ...
    private void autoArmControl() {
        ...
        if (armState == ArmState.RESTING_DOWN && gamepad1.a && prevG2A){
        ...
        if (armState == ArmState.RESTING_UP && gamepad1.a && prevG2A){
        ...
        if (autoArmDown) {
            ...
            arm.setPower(ARM_DOWN_SPEED);
            if(armPosition < ARM_MIN_POSITION + 500) arm.setPower(-ARM_SLOW_SPEED);
    ...
    private void driveControl() {
        ...
        if (throttle < STICK_THRESHOLD && throttle > -STICK_THRESHOLD
            ...
            throttle = reverseMult * -gamepad2.left_stick_y;
        ...
        telemetry.addData("gamepad1.right_trigger", String.format("%.2f", gamepad1.right_trigger));
